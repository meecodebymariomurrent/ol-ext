...
var ol_control_Timeline = function (options) {
    ...
    this._scrollDiv.addEventListener('scroll', function () {
        ...
        scrollListener = setTimeout(function () {
            ...
            const dayEqual = dateEnd.getDay() === updateFinishedDate.getDay();
            const dateEqual = yearEqual && monthEqual && dayEqual;
            if (!this._updatingDate && !this._initializing) {
                this.dispatchEvent({
                    type: 'scroll',
                    date: this.getDate(),
...
ol_control_Timeline.prototype.setDate = function (feature, options) {
    ...
    if (feature instanceof Date) {
        this._updatingDate = true;
        this._programmaticallySetDate = feature;
        date = feature;
    } else {
